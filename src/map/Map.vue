...
export default {
  ...
  watch: {
	...
	defaultCursor(val){
	  const { map } = this;
	  map.setDefaultCursor(global[val]);
	},
	draggingCursor(val){
	  const { map } = this;
	  map.setDraggingCursor(global[val]);
	},
    dragging (val) {
      const {map} = this
  ...
  methods: {
    setMapOptions () {
      const {map, minZoom, maxZoom, mapType,defaultCursor,draggingCursor, dragging, scrollWheelZoom, doubleClickZoom, keyboard, inertialDragging, continuousZoom, pinchToZoom, autoResize} = this
      minZoom && map.setMinZoom(minZoom)
      maxZoom && map.setMaxZoom(maxZoom)
      mapType && map.setMapType(global[mapType])
	  defaultCursor && map.setDefaultCursor(global[defaultCursor])
	  draggingCursor && map.setDraggingCursor(global[draggingCursor])
